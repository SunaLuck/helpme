...
token="6817638740:AAHID7nVeoSQyLCThlbE9EYtdDUhOSuJ2R8"
...
def one_day_tt(message):
    ...
    result = termtables.to_string(records,
                                header=["№ пары", "Начало", "Предмет", "Аудитория"],
...
def start(message):
    ...
    keyboard.row("/timetable")
    bot.send_message(message.chat.id, 'Это бот с расписанием для группы БВТ2202' , reply_markup=keyboard)
